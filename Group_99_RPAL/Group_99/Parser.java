...
class Lexical {
    ...
    private ArrayList<String> punctuation = new ArrayList<String>(); // Punctuations
    ...
    Lexical(String fileName) {
        ...
        s = new String[] { "(", ")", ";", "," };
        punctuation.addAll(Arrays.asList(s));
        s = new String[] { "let", "in", "fn", "where", "aug", "or", "not", "gr", "ge", "ls", "le", "eq", "ne", "within",
                "and", "rec" };
    ...
    // Parse the current line and select a token to return. If EndOfLine reached,
    // make line = null for next line reading.
    ...
    private String parse() {
        ...
        do {
            ...
            // PUNCTUATIONS
            else if (punctuation.contains(line.charAt(0) + "")) {
                type = "<PUNCTUAION>";
                sb.append(line.charAt(0));
                line = line.substring(1);
                if (line.length() == 0)
...
class Parser {
    ...
    private void read(String s) {
        ...
        if ((!l.reserved.contains(token)) && (l.getType().equals("<IDENTIFIER>") || l.getType().equals("<INTEGER>")
                || l.getType().equals("<STRING>")))
            buildTree(token, 0); // Build a node in case of identifiers, integers and strings
        token = l.getToken();
    }

    // Build Tree by popping the specified number of trees from stack and join with
    // root node specified
    ...
    private void buildTree(String root, int n) {
        ...
        while (i < n) {
            Node sib = stack.pop();
            sib.setSibling(temp); // First popped node has sibling set as null, second popped has sibling as
                                  // 'first popped'
            temp = sib;
            i++;
        }
    ...
    private void R() {
        Rn();
        while ((!l.reserved.contains(token)) && (l.getType().equals("<IDENTIFIER>") || l.getType().equals("<INTEGER>")
                || l.getType().equals("<STRING>") ||
                token.equals("true") || token.equals("false") || token.equals("nil") || token.equals("(")
                || token.equals("dummy"))) {
            Rn();
            buildTree("gamma", 2);
        }
    ...
    private void preOrder(Node n, int level, StringBuilder AST) {
        String dot = "";
        for (int i = 0; i < level; i++)
            dot = dot + ".";

        AST.append(dot).append(n.getToken()).append("\n");
        if (n.getChild() != null) {
            preOrder(n.getChild(), level + 1, AST);
        }
        if (n.getSibling() != null) {
            preOrder(n.getSibling(), level, AST);
        }
